...
function addSharedFiles(xcodeProject, { sharedFiles, watchTargetUuid }) {
    ...
    // 1. Shared 그룹 생성
    const { uuid: sharedGroupUuid } = xcodeProject.addPbxGroup([], '"Shared"', sharedPath);
    // 루트 그룹에 추가
    ...
    sharedFiles.forEach((fileName) => {
        ...
        xcodeProject.hash.project.objects['PBXBuildFile'][`${iosBuildUuid}_comment`] = `${fileName} in Sources`;
        ...
        xcodeProject.hash.project.objects['PBXBuildFile'][`${watchBuildUuid}_comment`] = `${fileName} in Sources`;
        // Build Phase에 추가
        addToSourcesBuildPhase(xcodeProject, iosTargetUuid, iosBuildUuid, fileName);
        addToSourcesBuildPhase(xcodeProject, watchTargetUuid, watchBuildUuid, fileName);
    });
}
function addToSourcesBuildPhase(xcodeProject, targetUuid, buildFileUuid, fileName) {

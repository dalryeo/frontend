...
extension WorkoutManager {
  ...
  public func prepareWorkout(with externalConfig: HKWorkoutConfiguration? = nil) async throws {
    ...
    do {
      ...
      WorkoutLogger.warning("미러링 1차 실패, 재시도 중...: \(error.localizedDescription)")
      // 0.5초 후 한 번 더 시도
...
extension WorkoutManager: WCSessionDelegate {
  func session(_ session: WCSession, activationDidCompleteWith activationState: WCSessionActivationState, error: Error?) {
    WorkoutLogger.debug("Watch WCSession 활성화 완료 상태: \(activationState.rawValue)")
  }
  
  // 아이폰에서 sendMessage(["request": "ping"], ...)를 보냈을 때 호출됨
  func session(_ session: WCSession, didReceiveMessage message: [String : Any], replyHandler: @escaping ([String : Any]) -> Void) {
    if message["request"] as? String == "ping" {
      WorkoutLogger.debug("아이폰으로부터 핑 수신 -> 퐁 응답 전송")
